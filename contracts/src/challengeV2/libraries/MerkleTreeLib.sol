...
library MerkleTreeLib {
    ...
    function verifyInclusionProofStart(bytes32 rootHash, bytes32 leaf, uint256 index, bytes32[] memory proof)
        internal
        pure
    {
        bytes32 calculatedRoot = MerkleLib.calculateRoot(proof, index, keccak256(abi.encodePacked(leaf)));
        require(rootHash == calculatedRoot, "Invalid inclusion proof start");
    }
}
