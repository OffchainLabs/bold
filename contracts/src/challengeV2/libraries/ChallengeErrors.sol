...
error ClaimEdgeInvalidLevel(uint64 argLevel, uint64 claimLevel);
...
error EdgeTypeNotBlock(uint64 level);
/// @dev The edge is not of type SmallStep
error EdgeTypeNotSmallStep(uint64 level);
...
error EdgeLevelInvalid(bytes32 edgeId1, bytes32 edgeId2, uint64 level1, uint64 level2);
...
error LevelTooHigh(uint64 level, uint64 numBigStepLevels);
