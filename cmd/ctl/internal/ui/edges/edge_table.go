...
func (e *EdgesTableContent) GetCell(row, column int) *tview.TableCell {
  ...
  case ColumnTopLevelClaimHeights:
		if ee.TopLevelClaimHeight == nil || ee.TopLevelClaimHeight.ChallengeOriginHeights == nil {
			str = "MISSING: TopLevelClaimHeight"
		} else {
      // json marshal works well enough to print as comma seperated string
      s, err := json.Marshal(ee.TopLevelClaimHeight.ChallengeOriginHeights)
      if err != nil {
        str = err.Error()
        break
      }
			str = fmt.Sprintf("%v", strings.Trim(string(s), "[]")) // Remove brackets
		}
	default:
...
var (
  ...
  ColumnTopLevelClaimHeights = "TopLevelClaimHeights"
)
