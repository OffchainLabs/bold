...
func TestAssertionChain_BlockChallenge_CreateLeafInvariants(t *testing.T) {
	...
	t.Run("ineligible for new successor", func(t *testing.T) {
		...
		rootVertex := &ChallengeVertex{
			PresumptiveSuccessorV: util.Some(protocol.ChallengeVertex(&ChallengeVertex{
				PsTimerV: timer,
			})),
...
func TestAssertionChain_Bisect(t *testing.T) {
	...
	err := assertionsChain.Tx(func(tx protocol.ActiveTx) error {
		...
		// require.NoError(t, err)
		correctBranch, err := assertionsChain.CreateAssertion(ctx, tx, 6, 0, nil, nil, nil)
		require.NoError(t, err)
		// wrongBranch, err := assertionsChain.CreateLeaf(tx, genesis, util.StateCommitment{Height: 6, StateRoot: wrongBlockHashes[6]}, staker2)
		wrongBranch, err := assertionsChain.CreateAssertion(ctx, tx, 6, 0, nil, nil, nil)
		require.NoError(t, err)

		// TODO: Staker 2.
		challenge, err := assertionsChain.CreateSuccessionChallenge(ctx, tx, 0)
		require.NoError(t, err)
...
func TestAssertionChain_Merge(t *testing.T) {
	...
	t.Run("past deadline", func(t *testing.T) {
		...
		ps := util.Some(protocol.ChallengeVertex(&ChallengeVertex{
			PsTimerV: counter,
			Commitment: util.HistoryCommitment{
				Height: 1,
			},
...
func TestAssertionChain_ChallengePeriodLength(t *testing.T) {
	...
	resp, err := chain.ChallengePeriodSeconds(ctx, tx)
	require.NoError(t, err)
	require.Equal(t, resp, cp)
}
...
func TestAssertionChain_RetrieveAssertions(t *testing.T) {
	...
	got, err = chain.AssertionBySequenceNum(ctx, tx, 1)
	require.NoError(t, err)
...
func TestAssertion_ErrWrongState(t *testing.T) {
	...
	chain := NewAssertionChain(ctx, util.NewArtificialTimeReference(), testChallengePeriod)
	tx := &ActiveTx{TxStatus: ReadWriteTxStatus}
	as, err := chain.LatestConfirmed(ctx, tx)
	require.NoError(t, err)
	a := as.(*Assertion)
...
func TestAssertion_HasConfirmedSibling(t *testing.T) {
	...
	c.rootAssertion.Unwrap().challengeManager.challengeVerticesByCommitHash[protocol.ChallengeHash(h)] = map[protocol.VertexHash]*ChallengeVertex{
		protocol.VertexHash(h): &ChallengeVertex{SequenceNumV: 100, StatusV: protocol.AssertionConfirmed, PrevV: util.Some(protocol.ChallengeVertex(parent))},
	}

	child := &ChallengeVertex{SequenceNumV: 101, PrevV: util.Some(protocol.ChallengeVertex(parent))}

	hasConfirmedSibling, _ := child.HasConfirmedSibling(ctx, tx)
