...
func TestChallengeBoundaries_DifferentiateAssertionAndExecutionStates(t *testing.T) {
	...
	blockChalCommit, err := manager.HistoryCommitmentAtMessage(ctx, 4)
	require.NoError(t, err)
...
func TestGranularCommitments_SameStartHistory(t *testing.T) {
	...
	start, err := manager.HistoryCommitmentAtMessage(ctx, fromBlockChallengeHeight)
	require.NoError(t, err)
	end, err := manager.HistoryCommitmentAtMessage(ctx, toBlockChallengeHeight)
	require.NoError(t, err)
...
func TestAllPrefixProofs(t *testing.T) {
	...
	require.NoError(t, err)
	hiCommit, err := manager.HistoryCommitmentAtMessage(ctx, to)
	require.NoError(t, err)
...
func TestPrefixProofs(t *testing.T) {
	...
	for _, c := range []struct {
		...
		postExpansion, err := manager.HistoryCommitmentAtMessage(ctx, c.hi)
		require.NoError(t, err)
