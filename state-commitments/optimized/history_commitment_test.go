...
func TestPrefixProofGeneration(t *testing.T) {
	...
	for _, tt := range tests {
		for prefixIndex := 3; prefixIndex < tt.virtualLength; prefixIndex++ {
			t.Run(fmt.Sprintf("real length %d, virtual %d, limit %d, prefix index %d", tt.realLength, tt.virtualLength, tt.limit, prefixIndex), func(t *testing.T) {
				legacy := computeLegacyPrefixProof(t, ctx, tt.virtualLength, prefixIndex)
				optimized := computeOptimizedPrefixProof(t, tt.realLength, tt.virtualLength, tt.limit, prefixIndex)
				verify(t, legacy)
				verify(t, optimized)
			})
		}
