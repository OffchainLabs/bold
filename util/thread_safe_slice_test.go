...
func TestThreadSafeSlice(t *testing.T) {
	t.Run("empty slice", func(t *testing.T) {
		...
		require.Equal(t, EmptyOption[int](), s.Get(0))
		require.Equal(t, EmptyOption[int](), s.Last())
	...
	t.Run("appending to empty", func(t *testing.T) {
		...
		s.Append(1)
		require.Equal(t, FullOption[int](1), s.Get(0))
		require.Equal(t, FullOption[int](1), s.Last())
	...
	t.Run("appending to existing", func(t *testing.T) {
		...
		s.Append(4)
		require.Equal(t, FullOption[int](1), s.Get(0))
		require.Equal(t, FullOption[int](4), s.Last())
		require.Equal(t, false, s.Empty())
